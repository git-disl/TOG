...
from frcnn_utils.utils.bbox_tools import loc2bbox
from frcnn_utils.utils.nms import non_maximum_suppression
...
class FasterRCNN(nn.Module):
    ...
    def _suppress(self, raw_cls_bbox, raw_prob, raw_logits):
        bbox = list()
        label = list()
        logits = list()
        score = list()
    ...
    def predict(self, imgs, score_thresh, nms_thresh):
        ...
        for img, size in zip(prepared_imgs, sizes):
            ...
            bbox, label, score, logit = self._suppress(raw_cls_bbox, raw_prob, raw_logit)
            bboxes.append(bbox)
            labels.append(label)
            scores.append(score)
            logits.append(logit)
    ...
    def get_optimizer(self):
        """
        return optimizer, It could be overwritten if you want to specify special optimizer
        """
        lr = opt.lr
        params = []
