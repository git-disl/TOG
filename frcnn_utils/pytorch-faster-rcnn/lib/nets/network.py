...
from lib_utils.visualization import draw_bounding_boxes
...
class Network(nn.Module):
    ...
    def _proposal_layer(self, rpn_cls_prob, rpn_bbox_pred):
        rois, rpn_scores, all_rpn_bboxes = proposal_layer( \
            rpn_cls_prob, rpn_bbox_pred, self._im_info, self._mode,
            self._feat_stride, self._anchors, self._num_anchors)

        return rois, rpn_scores, all_rpn_bboxes
    ...
    def _region_proposal(self, net_conv):
        ...
        if self._mode == 'TRAIN':
            rois, roi_scores, _ = self._proposal_layer(rpn_cls_prob, rpn_bbox_pred)  # rois, roi_scores are varible
            rpn_labels = self._anchor_target_layer(rpn_cls_score)
            rois, _ = self._proposal_target_layer(rois, roi_scores)
        else:
            if cfg.TEST.MODE == 'nms':
                rois, _, all_rpn_bboxes= self._proposal_layer(rpn_cls_prob, rpn_bbox_pred)
            elif cfg.TEST.MODE == 'top':
                rois, _, all_rpn_bboxes = self._proposal_top_layer(rpn_cls_prob, rpn_bbox_pred)
            else:
        ...
        self._predictions["all_rpn_bboxes"] = all_rpn_bboxes
    ...
    def forward(self, image, im_info, gt_boxes=None, mode='TRAIN'):
        ...
        self._image = image  #torch.from_numpy(image.transpose([0, 3, 1, 2])).to(self._device)
    ...
    def test_image(self, image, im_info):
        self.eval()
        with torch.no_grad():
            image = torch.from_numpy(image.transpose([0, 3, 1, 2])).float().to(self._device)
            self.forward(image, im_info, None, mode='TEST')
        cls_score, cls_prob, bbox_pred, rois = self._predictions[
                                                   "cls_score"].data.cpu().numpy(), \
                                               self._predictions[
                                                   'cls_prob'].data.cpu().numpy(), \
                                               self._predictions[
                                                   'bbox_pred'].data.cpu().numpy(), \
                                               self._predictions[
                                                   'rois'].data.cpu().numpy()
        return cls_score, cls_prob, bbox_pred, rois

    # only useful during testing mode
    def test_image_adv(self, image, im_info):
        self.eval()
        image = torch.from_numpy(image.transpose([0, 3, 1, 2])).to(self._device)
        image = torch.autograd.Variable(image, requires_grad=True)
        self.forward(image, im_info, None, mode='TEST')
        cls_score, cls_prob, bbox_pred, rois, rpn_cls_prob, rpn_bbox_pred, all_rpn_bboxes = self._predictions[
                                                                                "cls_score"].data.cpu().numpy(), \
                                                                            self._predictions[
                                                                                'cls_prob'].data.cpu().numpy(), \
                                                                            self._predictions[
                                                                            ...
                                                                            self._predictions[
                                                                                'rpn_bbox_pred'], \
                                                                            self._predictions['all_rpn_bboxes']

        return cls_score, cls_prob, bbox_pred, rois, rpn_cls_prob, rpn_bbox_pred, all_rpn_bboxes
