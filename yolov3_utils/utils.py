...
def letterbox_image(image, size, extended=False, to_numpy=False):
    ...
    if to_numpy:
        return_image = np.expand_dims(np.array(new_image, dtype='float32') / 255., 0)
    if extended:
        return return_image, (w - nw) // 2, (h - nh) // 2, nw + (w - nw) // 2, nh + (h - nh) // 2, scale
    return return_image
...
def bb_intersection_over_union(boxA, boxB):
    ...
    # of prediction + ground-truth areas - the interesection area
